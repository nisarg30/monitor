...
//     try {
//         const response = await axios.post("http://localhost:8080/nifty_1", {
//             csvData: csvContent,
//             stockName: stockName,
//             tf: tf,
//             dir: dir
...
async function writeCandlestickDataToServer(stockName, confirmationPeriods, chartPeriods, tf, dir) {
    ...
    try {
        const response = await axios.post("http://localhost:8080/nifty_5", {
            csvData: csvContent,
            stockName: stockName,
            tf: tf,
            dir: dir
        ...
        const query = stockName + '_' + tf;
        const filter = { stockname: query };
        const logEntry = {
            time: chartPeriods[0].time,
            direction: dir,
            result: response.data[0]
        ...
        // Send notification if response.data is true
        if (response.data) {
            const detectionType = dir === 1 ? 'Shape' : 'Shape 2';
            notifier.notify({
                title: 'Trading Alert',
                message: `Stock: ${stockName}_${tf}\nResult: ${response.data[0]}\nDetection: ${detectionType}`,
                sound: true
...
const xyz = async (stockname, tf) => {
    ...
    chart.onUpdate(async () => {
        ...
        if (ConfirmationEntry.periods[1].Shapes === 1 && last != chart.periods[1].time) {
            ...
            writeCandlestickDataToServer(chart.infos.name, ConfirmationEntry.periods.splice(0, 200), chart.periods.splice(0, 200), '5', 1);
        } else if (ConfirmationEntry.periods[1].Shapes_2 === 1 && last != chart.periods[1].time) {
            logWithTimestamp('Shape 2 detected');
            last = chart.periods[1].time;
            writeCandlestickDataToServer(chart.infos.name, ConfirmationEntry.periods.splice(0, 200), chart.periods.splice(0, 200), '5', 0);
        }
    });
};
